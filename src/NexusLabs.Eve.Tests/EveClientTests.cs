...
public sealed class EveClientTests
{
    ...
    public async Task GetInfoAsync_AcceptsSchemaVersionFourKernelEffectActionAndPreservesRaw(
        string action,
        CancellationToken cancellationToken)
    {
        EveAgentInfo info = await GetInfoAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        await Assert.That(
                info.Raw.GetProperty("kernelEffects")[0].GetProperty("action").GetString())
