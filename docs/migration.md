...
!!! success "eve 0.32.0 through 0.44.0 need no migration"
    None of these releases broke the framework-neutral client protocol. The core session
    routes used by this package are unchanged across `0.31.0` through `0.44.0`.
    `0.34.0` added `approval.candidate` and `approval.settled`; later releases through
    `0.44.0` remove no stream event type and add only additive fields and events.
    ...
    the typed .NET request model already enforces. Eve `0.43.0` and `0.44.0` add no further
    framework-neutral requirement. These later changes require no deployment migration.
